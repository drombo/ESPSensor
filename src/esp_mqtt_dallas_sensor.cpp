...
/*
flash mode: hold flash button (P) and press reset
To enable Config Access Point Press reset an then press flash button (P) within 5 seconds
After that. Find the AccessPoint ESPConfigAP and connect.
*/
...
#define BUTTON_PRESS_DELAY 5      // Time to press flash button after reset in s
#define WIFI_CONNECT_TIME 4         // time to connect to wifi in s
...
void setup() {
	...
	delay(BUTTON_PRESS_DELAY*1000); // time to press the flash button
	if (mqtt_server == "" || digitalRead(TRIGGER_PIN) == LOW) {
		Serial.println("Start ConfigAP");
	...
	sensors.begin();
	sensors.setResolution(12); // Genauigkeit auf 12-Bit setzen
	...
	DEBUG_PRINTLN("");
	DEBUG_PRINT(
			"go to sleep for "+String((SLEEPTIME - WIFI_CONNECT_TIME - BUTTON_PRESS_DELAY), DEC));
	DEBUG_PRINTLN("sec");
	ESP.deepSleep((SLEEPTIME - WIFI_CONNECT_TIME - BUTTON_PRESS_DELAY) * 1000000);
}
