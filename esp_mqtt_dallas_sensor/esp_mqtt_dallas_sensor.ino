...
boolean readDallasSensor(DeviceAddress deviceAddress) {
  ...
  if (mqttclient.publish( (char*) t_topic.c_str(), (char*) t_payload.c_str())) {
    ...
    Serial.println("Publish temperature failed for sensor " + getAddress(deviceAddress));
  }
...
void setup() {  
  ...
  for (int i = 0; i < MAX_NUMBER_OF_SENSORS; i++) {
    if (!sensors.getAddress(sensor[i], i)) {
      ...
      activeSensors++;
      Serial.print("Device "); Serial.print(i, DEC); Serial.print(" Address: ");
  ...
  if (mqttConnect()) {
    mqttclient.publish((char*) infoTopic.c_str(), "Start reading DS18B20 sensors");
